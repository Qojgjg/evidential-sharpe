...
class TSTransformerEncoder(nn.Module):
    ...
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, dropout=0.1,
                 pos_encoding='fixed', activation='gelu', norm='BatchNorm', freeze=False, **kwargs):
...
class TSTransformerEncoderClassiregressor(nn.Module):
    ...
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, num_classes,
                 ...
                 logits_converter=relu_evidence, **kwargs):
        ...
        self.logits_converter = logits_converter
    ...
    def forward(self, X, padding_masks=None):
        ...
        """
        padding_masks = torch.ones(X.shape[0], X.shape[1], dtype=torch.long, device=torch.device("cuda"))
        # permute because pytorch convention for transformers is [seq_length, batch_size, feat_dim]. padding_masks [batch_size, feat_dim]
        inp = X.permute(1, 0, 2)
        inp = self.project_inp(inp) * math.sqrt(
            self.d_model)  # [seq_length, batch_size, d_model] project input vectors to d_model dimensional space
        ...
        evidence = self.logits_converter(output)  # (batch_size, num_classes)
